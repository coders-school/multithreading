...
T p_accumulate(Iterator first, Iterator last, T init){
    ...
    {
        Iterator end = std::next(begin_it, data_chunk);
...
int main() {
    ...
    auto result1 = std::accumulate(begin(vec), end(vec), 0) <<std::endl;
    auto stop1 = std::chrono::high_resolution_clock::now();

    auto start2 = std::chrono::high_resolution_clock::now();
    auto result2 = std::accumulate(std::execution::par, begin(vec), end(vec), 0) <<std::endl;
