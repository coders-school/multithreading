...
int main() {
    ...
    auto sumOfvectorParalel = pararelAccumulate(std::begin(vec), std::end(vec), 0);
    ...
    auto sumOfvec = std::accumulate(std::begin(vec), std::end(vec), 0);
    stop = std::chrono::high_resolution_clock::now();
    std::cout<< "End of std::accumulate. \n";
    std::cout<< "Time of std::accumulate function: "
             << std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count() << "\n\n";

    assert(sumOfvectorParalel == sumOfvec);
