...
int main() {
    ...
    __gnu_parallel::accumulate(std::begin(vec), std::end(vec), 0);
	...
	std::cout << "\nNormal algorithm: "
		...
		<< "us" << std::endl;
    std::cout << "\nNormal algorithm with execution policy: "
		<< std::chrono::duration_cast<std::chrono::microseconds>(stop5 - start5).count() 
		<< "us" << std::endl;
